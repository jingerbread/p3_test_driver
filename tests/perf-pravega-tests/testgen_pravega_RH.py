...
image = 'devops-repo.isus.emc.com:8116/maria/omb:dev2.6.0-1c40f1e'
    ...
    for producerWorkers in [2]:
        ...
        for testDurationMinutes in [2]:
            for messageSize in [10000]:
                for producerRateEventsPerSec in [7e4, 5e2, 55e3, 7e3, 60e3, 1e3, 50e3, 1e4, 6e3, 3e3, 5e3, 45e3, 9e3, 4e4, 15e3, 3e4, 25e3, 35e3, 2e4]:
                    for topics in [4]:
    ...
    for producerWorkers in [2]:
        ...
        for testDurationMinutes in [2]:
            for messageSize in [100]:
                for producerRateEventsPerSec in [-1, 25e3, 15e5, 1e5, 1e6, 25e4, 9e5, 1e2, 5e5, 1e3, 6e5, 1e4, 6e4, 5e3, 5e4]:
                    for topics in [4]:
                        for partitionsPerTopic in [16]:
                            for producersPerWorker in [2]:
                                producersPerTopic = int(producersPerWorker * producerWorkers)
