...
class OpenMessagingBenchmarkSSHTest(BaseTest):
    ...
    def run_test(self):
        ...
        if localWorker:
            ...
            return_code, results_yaml, errors = self.ssh('cat /opt/no_netty_benchmark/workers.yaml')
        ...
        if driver['name'] == 'Pravega':
            return_code, results_yaml, errors = self.ssh('cat /opt/no_netty_benchmark/driver-pravega/pravega.yaml')
            ...
            return_code, results_yaml, errors = self.ssh('cat /opt/no_netty_benchmark/driver-pulsar/pulsar.yaml')
            deployed_driver = yaml.load(StringIO(results_yaml))
            ...
            return_code, results_yaml, errors = self.ssh('cat /opt/no_netty_benchmark/driver-kafka/kafka.yaml')
            deployed_driver = yaml.load(StringIO(results_yaml))
        ...
        try:
            return_code, results_json, errors = self.ssh(
                'cat /opt/no_netty_benchmark/*' + test_uuid + '*.json',
                print_output=False,
            )
